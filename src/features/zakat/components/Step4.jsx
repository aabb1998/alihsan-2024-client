  ...
  return (
    <div className="flex flex-col gap-5 sm:gap-10">
      <div className="form-group">
        ...
        {errors.loan && <div className="mt-1 text-sm text-red-300">{errors.loan}</div>}
        ...
        {errors.other && <div className="mt-1 text-sm text-red-300">{errors.other}</div>}
      </div>
      <div className="flex gap-3">
        <Button variant={"secondary"} label={"Back"} onClick={() => dispatch(zakatStep(-1))} />
        <Button variant={"primary"} label={"Done"} onClick={() => dispatch(zakatStep(1))} />
      </div>
    </div>
  );
};
