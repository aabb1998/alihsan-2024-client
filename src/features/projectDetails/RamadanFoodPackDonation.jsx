...
const RamadanFoodPackDonation = ({ campaign, handleClose, isModal }) => {
  ...
  const findOption = (option) => {
    let finalOption;
    if (option === "1") {
      finalOption = "Feed 1 Family";
    } else if (option === "5") {
      finalOption = "Feed 5 Families";
    } else if (option === "10") {
      finalOption = "Feed 10 Families";
    } else if (option === "20") {
  ...
  const handleQuantityChange = (e, type) => {
    e.preventDefault();
    formik.setFieldValue("amount", "12");
    if (type === "add") {
      ...
      <form onSubmit={formik.handleSubmit}>
        <div className="flex flex-col gap-5 md:gap-8">
          ...
          <p className="px-4 py-3 font-bold text-sm text-neutral-800 rounded-lg bg-neutral-200">
            {
              "Through your generous contributions, we aim to provide essential food packs to families facing hardships. Feed each family for $12."
            }
          </p>
...
export default RamadanFoodPackDonation;

RamadanFoodPackDonation.propTypes = {
  isModal: PropTypes.bool,
};

RamadanFoodPackDonation.defaultProps = {
  handleClose: () => null,
  isModal: false,
};
