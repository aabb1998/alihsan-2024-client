...
const RadioButtonItems = [
    ...
    },
    {
        ...
        description: {
            ...
            para2: "The one who breaks his fast with no excuse must repent to Allah, regret what he has done, resolve to never do it again, and do a lot of righteous deeds such as giving in charity, observing nafl fasts and so on."
        },
        costforOneDay: false,
        costPerPerson: false,
        counter: false,
    ...
    },
    {
        ...
        description: {
            title: "Fast 3 Days:",
            para1: "It is not allowed to resort to fasting if you can perform one of the above."
        }
    },
    {
        type: "vows",
...
export const FedyahDonation = ({ campaign, handleClose, isModal }) => {
    ...
    const handleChangeCategory = (e) => {
        ...
        setCategory(value)
        const filteredItem = RadioButtonItems?.filter(items => items?.type === value)
    ...
    const handleRadioButton = (e) => {
        ...
        setQuantity(1)
        const { value } = e.target
        const selectedItem = RadioButtonItems?.find(items => items?.title === value)
    ...
    const formik = useFormik({
        initialValues: {
            ...
            checkoutType: "FEDYAH",
            quantity: quantity === 1 ? (subItem?.quantity || selectedTab?.quantity) : quantity,
    ...
    return (
        <div className={isModal ? 'h-full bg-white' : 'border rounded-4xl border-neutral-300 px-4 py-6 md:p-7.5 bg-white'}>
            <form onSubmit={formik.handleSubmit} className='flex flex-col justify-between h-full'>
                {!isModal ? <div className="flex items-center justify-between mb-5 md:mb-7.5">
                ...
                <div className="mb-5 md:mb-7.5">
                    <div className="form-group">
                        ...
                        >
                            ...
                            <option value="kaffarah">Kaffarat al Yamin (Break Deliberate Oath)</option>
                ...
                {filteredData.length ?
                    <div className='mb-5 md:mb-7.5'>
                        {filteredData?.map((item, index) => (
                            <div key={index} className='flex items-center justify-between gap-2 pr-3 mb-3 sm:pr-1'>
                                <div className='flex items-center gap-2'>
                                    <input type="radio" name={item?.type} checked={index === selectedRadio} value={item?.title} onChange={handleRadioButton} />
                                ...
                                <div className="cursor-pointer text-neutral-700 hover:text-primary-300" data-tooltip-id={`my-tooltip${index}`} data-tooltip-place="bottom-end">
                                    ...
                                    <Tooltip id={`my-tooltip${index}`} className="opacity-100 tooltip" style={{ backgroundColor: "#fff", padding: "1rem", zIndex: '999', opacity: '1', }}>
                                        <div>
                                            <h2 className="mb-2 text-neutral-1000 text-button-md">{item?.description?.title}</h2>
                                            <p className="text-xs font-medium text-neutral-600">{item?.description?.para1}</p>
                                            {item?.description?.subTitle1 && <><h4 className="text-xs font-medium text-neutral-600">1 {item?.description?.subTitle1}</h4></>}
                    ...
                    {subItem?.initialAmount ? (
                        <div className='flex items-baseline justify-between mb-7.5'>
                            ...
                            {subItem?.counter && <div className="flex flex-col">
                                <h4 className='mb-2 text-sm !font-medium text-neutral-1000'>{subItem?.counterTitle}</h4>
                                <div>
                ...
                {subItem?.generalDonation && <div className='flex flex-row gap-3 mb-2'>
