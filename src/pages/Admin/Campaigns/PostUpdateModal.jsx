...
function addUrl(file) {
  if (file) file.url = URL.createObjectURL(file);
}
...
export default function PostUpdateModal() {
  ...
  const { id, campaignId, open, saving, details } = useSelector(
    ({ adminCampaigns }) => adminCampaigns.update
  );
    ...
    (state, action) => {
      if (action.type === "change") {
        ...
        if (action.name === "text") {
          ...
          if (val.length > 500)
            error = "Should contain less than 500 characters.";
        } else if (action.name === "images") {
          return {
            ...state,
            values: { ...state.values, images: [...state.values.images, val] },
            touched: { ...state.touched, images: true },
          };
        }
        return {
        ...
        if (action.target?.name === "images") {
          ...
          return {
            ...state,
            values: { ...state.values, images, deletedImages },
          };
        }
        ...
        const imageUrl = { url: action?.values?.images?.url,file:action?.values?.images?.file };
        return {
          ...state,
          values: {
            ...state.values,
            images: [...state.values.images, imageUrl],
          },
          touched: { ...state.touched, images: true },
        };
      }
  ...
  const submit = () => {
    ...
    for (let i of state.values.images) {
      if (!i.id) body.append("images", i?.file,`images.png`);
    }
  ...
  const getCroppedCoverImage = async (url) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        const imageList = state.values.images;
        setState({
          type: "add",
          values: {
            text: details?.text || "",
            images: { url: reader.result, file: file } || [],
          },
        });
  ...
  useEffect(() => {
    if (open)
      setState({
        type: "reset",
        values: { text: details?.text || "", images: details?.images || [] },
      });
  }, [details, open]);
            ...
            >
              <Dialog.Panel className="w-full max-w-modal-md transition-all transform p-5 sm:p-10 rounded-1.5xl bg-white">
                <div className="flex items-center justify-between mb-6">
                  ...
                  <button onClick={() => dispatch(closeUpdateModal())}>
                    <CloseIcon />
                  </button>
                </div>
                {isCropOpen && (
                ...
                <div className="max-h-[calc(100vh-17rem)] overflow-auto scroll-px-10 p-0  mb-7.5 border-0">
                  <form className="flex flex-col gap-4 sm:gap-5">
                    <div className="relative flex flex-col text-area">
                      <label
                        htmlFor="description"
                        className="text-sm font-medium sm:text-md text-neutral-1000"
                      >
                    ...
                    <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 sm:gap-4 md:gap-5">
                      {[
                        ...
                        <div
                          ...
                          for="dropzone-file"
                          className={
                            "relative flex flex-col items-center justify-center flex-grow border border-dashed rounded-lg cursor-pointer border-neutral-500" +
                            (image ? "" : "px-4 py-4 md:py-12 md:px-8")
                          }
                        ...
                        >
                          {image ? (
                            <>
                              ...
                              <div className="absolute flex items-center justify-center w-5 h-5 text-white bg-red-300 rounded-full top-2 right-2">
                                <CloseIcon
                                  iconSize={"16"}
                                  onClick={(e) => {
                                    setState({
                                      type: "remove",
                                      target: { name: "images", index },
                                    });
                                    e.stopPropagation();
                                  }}
                                />
                    ...
                    {id ? null : (
                      <div class="flex items-center mb-4">
                        <input
                          id="default-checkbox"
                          type="checkbox"
                          class="w-5 h-5 rounded-sm border border-neutral-300"
                          checked={state.values.emailDonors}
                          onChange={setState}
                          name="emailDonors"
                        />
                        <label
                          for="default-checkbox"
                          class="ms-2 font-Montserrat text-sm sm:text-md font-medium text-neutral-800"
                        >
                          Email Donors
                        </label>
                      </div>
                ...
                <div className="flex flex-wrap items-center justify-start gap-3 p-0 border-0 sm:gap-5">
                  <button
                    type="button"
                    className="flex-grow btn btn-outline-secondary sm:flex-grow-0"
                  ...
                  <button
                    type="button"
                    className="flex-grow btn btn-primary sm:flex-grow-0"
                    onClick={submit}
