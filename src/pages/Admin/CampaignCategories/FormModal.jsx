...
import {
  createCategory,
  editCategory,
} from "../../../features/adminCampaignCategories";
import FieldRequired from '../../../components/FieldRequired'

const ErrorLabel = ({ error }) => <div className="mt-2 text-red-300 text-sm">{error}</div>

const CategoryFormModal = ({ onClose, state }) => {
  const { rows, loader } = useSelector(
    (state) => state.adminCampaignCategories
  );
  useEffect(() => {
    if (state !== null && state !== true) {
      formState.dispatch({
        type: "reset",
        values: rows.find((i) => i.id === state),
      });
    } else if (state === null) {
  ...
  const formState = useFormState({
    ...
    onSubmit: ({ values }) => {
      let promise;
      if (state === true) promise = dispatch(createCategory(values));
      else promise = dispatch(editCategory({ payload: values, id: state }));
      promise.then((res) => {
  ...
  return (
    <div
      className={"relative z-10 " + (state === null ? "hidden" : "")}
              ...
              <div className="bg-white px-4 pt-4 pb-7.5 sm:px-10 sm:pt-10 sm:pb-10 py-10">
                <div className="flex flex-col gap-5 sm:gap-8">
                  ...
                  <div className="flex gap-5 sm:gap-7.5">
                    <Button
                      className="flex-grow basis-0 btn btn-outline-secondary"
                      variant=""
                      label={"Cancel"}
                      onClick={cancel}
                    />
                    <Button
                      className="flex-grow basis-0 btn btn-primary"
                      variant=""
                      label={"Submit"}
                      disabled={loader || (state !== true && !changed)}
                      onClick={submit}
                    />
                  </div>
                </div>
