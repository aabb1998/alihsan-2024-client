  ...
  function onSelectFile(e) {
    ...
    if (file && file.length > 0) {
      if (file[0].size > 3 * 1024 * 1024) {
        showErrorMessage("Image size should be less than 3MB");
        onClose();
        return;
      }
      ...
      <div>
        <div className="bg-white rounded-b-lg min-h-600">
          ...
          <div className="flex flex-col items-center justify-center">
            <div
              className={`w-full pb-5 ${cropped ? "hidden" : "opacity-100"}`}
            >
              {imgSrc && (
                <button
                  className="relative flex ml-auto cursor-pointer"
                  onClick={closeCropperScreen}
                >
                  <div className="pb-3">
                    <CloseIcon />
                  </div>
            ...
            {!!completedCrop && imgSrc && (
              <div
                className={`w-full my-3 flex justify-center mx-auto ${
                  cropped ? "opacity-100" : "hidden"
                }`}
              >
                <>
                  <canvas
