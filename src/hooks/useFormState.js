...
const makeInitialState = (schema, values) => {
  ...
  for (let i in schema) {
    state.errors[i] = schema[i].validator
...
const reducer = (state, action) => {
  if (action.type === "change") {
    ...
    if (
      ...
      val = target.value;
    else if (target.tagName === "ARRAYINPUT") val = target.array;
    ...
    const value = state.values[action.target.name].filter(
      (_, i) => i !== action.target.index
    );
    const sch = state.schema[action.target.name];
    const error = sch.validator
      ? sch.validator(value, state.values, state.touched) || ""
      : "";
    return {
      ...state,
...
export default function useFormState({ schema, onSubmit }) {
  ...
  const submit = (arg) => {
    ...
    onSubmit({ values: state.values, touched: state.touched, arg });
  };
  return {
    values: state.values,
    errors: state.errors,
